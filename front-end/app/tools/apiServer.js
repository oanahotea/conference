...
server.post("/meetings/", function (req, res, next) {
  const error = validatemeeting(req.body);
  if (error) {
    res.status(400).send(error);
  } else {
    req.body.slug = createSlug(req.body.name); // Generate a slug for new meetings.
    next();
  }
});
...
function validatemeeting(meeting) {
  if (!meeting.name) return "Name is required.";
  if (!meeting.speakerId) return "Speaker is required.";
  if (!meeting.date) return "Date is required.";
  return "";
}
